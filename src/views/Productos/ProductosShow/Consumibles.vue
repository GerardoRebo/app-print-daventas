<template>
  ...
  <v-card v-if="mdAndUp">
    <v-card-text>
      <p class="font-weight-bold">Productoo: {{ productActual.name }}</p>
  ...
  <v-container fluid>
    <v-data-table :headers="tHeaders" :items="consumibles" items-per-page="5">
      <template v-slot:item.codigo="{ item }">
        {{ item.consumible.codigo }}
      </template>
...
const posibleEnviar = computed(() => {
  return (
    productActual.value.id !== null && productEncontradoId.value !== null
...
function eliminarConsumible(consumible) {
  Product.eliminarConsumible(consumible)
    .then((response) => {
      getAll();
    })
    .catch((error) => {
...
function agregarConsumible() {
  ...
  processRequest(
    async () => {
      const { data } = await Product.agregarConsumible(
        productActual.value.id,
