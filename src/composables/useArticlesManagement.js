...
export function useArticlesManagement({
  ...
  function handleArticleEdit(articulo) {
    ...
    articulo_form.usa_medidas = !!articulo?.product?.usa_medidas;
    edicion.value = true;
  }
  ...
  function editarArticulo(updatedArticulo) {
    processRequest(
      ...
      cargando,
      {
        ...
        onError: (error) => notify.error(error.message || 'Error al actualizar artículo'),
        onFinally: () => {
  ...
  async function enviarArticulo() {
    if (product_form.usa_medidas && (!product_form.ancho || !product_form.alto)) {
      return notify.warning('Falta determinar: ancho o alto');
    }
