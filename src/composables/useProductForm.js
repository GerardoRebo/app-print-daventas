...
export function useProductForm(ticketActual, almacen, { onOpenConversion, cargandoRef } = {}) {
  ...
  // ============ State ============
  const codigo = ref('')
  const cargando = cargandoRef || ref(false)
  ...
  function rellenaProductForm(response) {
    ...
    product_form.cantidad_apartado = response.data.cantidad_apartado
  ...
  function emptyValues() {
    ...
    product_form.cantidad_apartado = ''
    priceOptions.value = []
  }
  ...
  /**
   * Selects price from modal and applies to product form
